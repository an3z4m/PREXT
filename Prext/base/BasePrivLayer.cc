...
bool BasePrivLayer::isInMixZone()
{
    ...
    {
        if (it->second.zoneType == 1)
        { // circular
            if (it->second.circularPos.distance(traci->getPositionAt(simTime())) <= it->second.circularRange)
            {
                if (mxzStartTime == 0)
    ...
    { //circular zone
        ...
        { // and I'm in this zone range already, then add it to the encountered zones
            if (mixZones.find(mzi.address) == mixZones.end())
            {
                ...
                if (mxzStartTime == 0)
                    mxzStartTime = simTime(); //but don't override start time
